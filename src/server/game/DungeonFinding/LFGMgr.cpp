...
{
    m_update = sWorld->getIntConfig(CONFIG_DUNGEON_FINDER_ENABLE);
    if (m_update)
    {
        new LFGPlayerScript();
        new LFGGroupScript();
...
void LFGMgr::LoadRewards()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_NETWORKIO,">> Loaded 0 lfg dungeon rewards. DB table `lfg_dungeon_rewards` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outInfo(LOG_FILTER_NETWORKIO, "Dungeon %u specified in table `lfg_dungeon_rewards` does not exist!", dungeonId);
            continue;
        }

        if (!maxLevel || maxLevel > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
        {
            sLog->outInfo(LOG_FILTER_NETWORKIO, "Level %u specified for dungeon %u in table `lfg_dungeon_rewards` can never be reached!", maxLevel, dungeonId);
            maxLevel = sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);
        }

        if (firstQuestId && !sObjectMgr->GetQuestTemplate(firstQuestId))
        {
            sLog->outInfo(LOG_FILTER_NETWORKIO, "First quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!", firstQuestId, dungeonId);
            firstQuestId = 0;
        }

        if (otherQuestId && !sObjectMgr->GetQuestTemplate(otherQuestId))
        {
            sLog->outInfo(LOG_FILTER_NETWORKIO, "Other quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!", otherQuestId, dungeonId);
            otherQuestId = 0;
        }
    ...
    sLog->outInfo(LOG_FILTER_NETWORKIO, ">> Loaded %u lfg dungeon rewards in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void LFGMgr::Update(uint32 diff)
{
                        ...
                        if (grp)
                         {
                            ...
                            player->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_PROPOSAL_BEGIN, GetSelectedDungeons(guid), GetComment(guid)),true);
                        }
                        else
                            player->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_PROPOSAL_BEGIN, GetSelectedDungeons(guid), GetComment(guid)),false);
    ...
    if (m_QueueTimer > LFG_QUEUEUPDATE_INTERVAL)
    {
        ...
        for (LfgQueueInfoMap::const_iterator itQueue = m_QueueInfoMap.begin(); itQueue != m_QueueInfoMap.end(); ++itQueue)
        {
            ...
            if (!queue)
            {
                /*sLog->outError("LFGMgr::Update: [" UI64FMTD "] queued with null queue info!", itQueue->first); to do */
                continue;
            }
...
void LFGMgr::Join(Player* player, uint8 roles, const LfgDungeonSet& selectedDungeons, const std::string& comment)
{
    ...
    if (grp)                                               // Begin rolecheck
    {
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            if (Player* plrg = itr->getSource())
            {
                ...
                plrg->GetSession()->SendLfgQueueStatus(rDungeonId, -1, m_WaitTimeAvg, m_WaitTimeTank, m_WaitTimeHealer, m_WaitTimeDps, -1, LFG_TANKS_NEEDED, LFG_HEALERS_NEEDED, LFG_DPS_NEEDED);
    ...
    else                                                   // Add player to queue
    {
        ...
        // Send update to player
        player->GetSession()->SendLfgJoinResult(joinData);
        player->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_JOIN_PROPOSAL, dungeons, comment),false);
        player->GetSession()->SendLfgQueueStatus(rDungeonId , waitTime, m_WaitTimeAvg, m_WaitTimeTank, m_WaitTimeHealer, m_WaitTimeDps, 0, pqInfo->tanks, pqInfo->healers, pqInfo->dps);
        ...
        case LFG_STATE_QUEUED:
            {
                ...
                {
                    player->GetSession()->SendLfgUpdateStatus(updateData,false);
                    ClearState(guid);
                }
            }
            break;
...
bool LFGMgr::CheckCompatibility(LfgGuidList check, LfgProposal*& pProposal)
{
    ...
    for (LfgGuidList::const_iterator it = check.begin(); it != check.end() && numLfgGroups < 2 && numPlayers <= MAXGROUPSIZE; ++it)
    {
        ...
        {
            sLog->outInfo(LOG_FILTER_NETWORKIO, "LFGMgr::CheckCompatibility: [" UI64FMTD "] is not queued but listed as queued!", (*it));
            RemoveFromQueue(guid);
            return false;
        }
    ...
    for (LfgRolesMap::const_iterator it = rolesMap.begin(); it != rolesMap.end(); ++it)
    {
        ...
        {
            for (PlayerPointerSet::const_iterator itPlayer = players.begin(); itPlayer != players.end() && player; ++itPlayer)
            {
    ...
    if (numPlayers != MAXGROUPSIZE)
    {
        ...
        for (PlayerPointerSet::const_iterator itPlayers = players.begin(); itPlayers != players.end(); ++itPlayers)
        {
            for (LfgQueueInfoMap::const_iterator itQueue = pqInfoMap.begin(); itQueue != pqInfoMap.end(); ++itQueue)
            {
                LfgQueueInfo* queue = itQueue->second;
    ...
    PlayerPointerSet::const_iterator itPlayers = players.begin();
    if (!leader)
    {
    ...
    for (LfgRolesMap::const_iterator it = roleCheck->roles.begin(); it != roleCheck->roles.end(); ++it)
    {
        ...
        switch (roleCheck->state)
        {
            ...
            case LFG_ROLECHECK_FINISHED:
                SetState(pguid, LFG_STATE_QUEUED);
                plrg->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE, dungeons, GetComment(pguid)),false);
                break;
            default:
                if (roleCheck->leader == pguid)
                    plrg->GetSession()->SendLfgJoinResult(joinData);
                plrg->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_ROLECHECK_FAILED),false);
                ClearState(pguid);
                break;
        }
    }
...
*/
void LFGMgr::GetCompatibleDungeons(LfgDungeonSet& dungeons, const PlayerPointerSet& players, LfgLockPartyMap& lockMap)
{
    lockMap.clear();
    for (PlayerPointerSet::const_iterator it = players.begin(); it != players.end() && !dungeons.empty(); ++it)
    {
    ...
    else
    {
        ...
        for (LfgPlayerList::const_iterator it = players.begin(); it != players.end(); ++it)
        {
            ...
            if (player->groupLowGuid != lowgroupguid)
                sLog->outInfo(LOG_FILTER_NETWORKIO, "LFGMgr::UpdateProposal: [" UI64FMTD "] group mismatch: actual (%u) - queued (%u)", (*it)->GetGUID(), lowgroupguid, player->groupLowGuid);
            ...
            if (itQueue == m_QueueInfoMap.end())
            {
                sLog->outInfo(LOG_FILTER_NETWORKIO, "LFGMgr::UpdateProposal: Queue info for guid [" UI64FMTD "] not found!", guid);
...
void LFGMgr::RemoveProposal(LfgProposalMap::iterator itProposal, LfgUpdateType type)
{
    ...
    for (LfgProposalPlayerMap::const_iterator it = pProposal->players.begin(); it != pProposal->players.end(); ++it)
    {
        ...
        else
        {
            ...
            {
                SetState(gguid, LFG_STATE_QUEUED);
                player->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE, GetSelectedDungeons(guid), GetComment(guid)),true);
            }
            else
                player->GetSession()->SendLfgUpdateStatus(LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE, GetSelectedDungeons(guid), GetComment(guid)),false);
        }
    }
...
{
    sLog->outTrace(LOG_FILTER_LFG, "LFGMgr::TeleportPlayer: [" UI64FMTD "] is being teleported %s", player->GetGUID(), out ? "out" : "in");
    ...
    else
    {
        ...
        // If players is in dungeon he can only exit
        ...
        else if (player->GetMapId() != uint32(dungeon->map))  // Do not teleport players in dungeon to the entrance
        {
            ...
            if (!mapid)
            {
                ...
                if (!at)
                {
                    sLog->outInfo(LOG_FILTER_NETWORKIO, "LfgMgr::TeleportPlayer: Failed to teleport [" UI64FMTD "]: No areatrigger found for map: %u difficulty: %u", player->GetGUID(), dungeon->map, dungeon->difficulty);
                    error = LFG_TELEPORTERROR_INVALID_LOCATION;
                }
                else
                {
            ...
            if (error == LFG_TELEPORTERROR_OK)
            {
                ...
                {
                    error = LFG_TELEPORTERROR_INVALID_LOCATION;
                    sLog->outInfo(LOG_FILTER_NETWORKIO, "LfgMgr::TeleportPlayer: Failed to teleport [" UI64FMTD "] to map %u: ", player->GetGUID(), mapid);
                }
            }
        }
    }
...
void LFGMgr::RewardDungeonDoneFor(const uint32 dungeonId, Player* player)
{
    ...
    // TODO: now if server crash LFG type is reset to 1 and it don't give reward. it should remember the type
...
time_t LFGMgr::GetJoinedTime(uint64 guid)
{
    LfgQueueInfoMap::iterator itQueue = m_QueueInfoMap.find(guid);
    if (itQueue != m_QueueInfoMap.end())
    {
        return itQueue->second->joinTime;
    }
    else
    {
        return NULL;
