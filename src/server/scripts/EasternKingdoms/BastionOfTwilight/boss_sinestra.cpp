...
enum Npc
{
    ...
    GO_CACHE_OF_THE_BROODMOTHER = 208044 // loot chest
};
        ...
        boss_sinestraAI(Creature* creature) : BossAI(creature, DATA_SINESTRA), summons(me)
        {
            ...
            introDone = false;
                ...
                void Reset()
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        Creature* egg = me->SummonCreature(NPC_PULSATING_EGG, EggLocations[i].GetPositionX(),
                                EggLocations[i].GetPositionY(), EggLocations[i].GetPositionZ(),
                                EggLocations[i].GetOrientation(), TEMPSUMMON_CORPSE_DESPAWN, 1000);
                ...
                void EnterPhaseTwo()
                {
                    ...
                    me->SummonCreature(NPC_SINESTRA_BAT_TRIGGER, MagicBattlePos[1].GetPositionX(),
                            MagicBattlePos[1].GetPositionY(), MagicBattlePos[1].GetPositionZ(),
                            MagicBattlePos[1].GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN);
                    me->SummonCreature(NPC_CALEN, -1004.784f, -810.12f, 438.593f, 0.91f, TEMPSUMMON_CORPSE_DESPAWN,
                            1000);
                }
                ...
                void JustDied(Unit* /*killer*/)
                {
                    ...
                    me->SummonGameObject(GO_CACHE_OF_THE_BROODMOTHER, -965.40f, -746.81f, 438.59f, 4.123275f, 0, 0, 0,
                            0, 300000);
                        ...
                        while (uint32 eventId = events.ExecuteEvent())
                        {
                            switch (eventId)
                            {
                                ...
                                case EVENT_WRACK:
                                    ...
                                    SelectTargetList(targets, NonTankTargetSelector(me), RAID_MODE(1, 1, 1, 2),
                                ...
                                case EVENT_TWILIGHT_SLICER:
                                    if (Creature* orb1 = me->SummonCreature(NPC_SHADOW_ORB,
                                            OrbLocation[0].GetPositionX(), OrbLocation[0].GetPositionY(),
                                            OrbLocation[0].GetPositionZ(), OrbLocation[0].GetOrientation(),
                                            TEMPSUMMON_CORPSE_DESPAWN, 1000))
                                        if (Creature* orb2 = me->SummonCreature(NPC_SHADOW_ORB,
                                                OrbLocation[1].GetPositionX(), OrbLocation[1].GetPositionY(),
                                                OrbLocation[1].GetPositionZ(), OrbLocation[1].GetOrientation(),
                                                TEMPSUMMON_CORPSE_DESPAWN, 1000))
                        ...
                        if (uint32 eventId = events.ExecuteEvent())
                        {
                            switch (eventId)
                            {
                                case EVENT_TWILIGHT_DRAKE:
                                    ...
                                    if (!left)
                                    {
                                        me->SummonCreature(NPC_TWILIGHT_DRAKE, WhelpsLocations[0].GetPositionX(),
                                                WhelpsLocations[0].GetPositionY(), WhelpsLocations[0].GetPositionZ(),
                                                WhelpsLocations[0].GetOrientation(), TEMPSUMMON_CORPSE_DESPAWN, 1000);
                                        left = true;
                                    }
                                    else
                                    {
                                        me->SummonCreature(NPC_TWILIGHT_DRAKE, WhelpsLocations2[0].GetPositionX(),
                                                WhelpsLocations2[0].GetPositionY(), WhelpsLocations2[0].GetPositionZ(),
                                                WhelpsLocations2[0].GetOrientation(), TEMPSUMMON_CORPSE_DESPAWN, 1000);
                                        left = false;
                                    }
                                ...
                                case EVENT_TWILIGHT_SPITECALLER:
                                    Talk(SAY_SPITECALL);
                                    me->SummonCreature(NPC_TWILIGHT_SPITE, me->GetPositionX() - 15, me->GetPositionY(),
                                            me->GetPositionZ(), 0, TEMPSUMMON_CORPSE_DESPAWN, 1000);
                                ...
                                case EVENT_MAGIC_BATTLE: // Insert dramatic music!!!
                                    if (Creature* trigger = me->FindNearestCreature(NPC_SINESTRA_BAT_TRIGGER, 200.0f,
                                            true))
...
class npc_twilight_drake: public CreatureScript
{
    public:
        ...
        npc_twilight_drake() :
                CreatureScript("npc_twilight_drake")
        {
        }
        ...
        struct npc_twilight_drakeAI: public ScriptedAI
        {
                ...
                npc_twilight_drakeAI(Creature * creature) :
                        ScriptedAI(creature)
                {
                    instance = creature->GetInstanceScript();
                }
...
class npc_twilight_spite: public CreatureScript
{
    public:
        ...
        npc_twilight_spite() :
                CreatureScript("npc_twilight_spite")
        {
        }
        ...
        struct npc_twilight_spiteAI: public ScriptedAI
        {
                ...
                npc_twilight_spiteAI(Creature * creature) :
                        ScriptedAI(creature)
                {
                    instance = creature->GetInstanceScript();
                }
                    ...
                    while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            ...
                            case EVENT_PYRRHIC_FOCUS:
                                if (me->GetPower(POWER_MANA) < (me->GetMaxPower(POWER_MANA) * 30 / 100)
                                        && !me->HasAura(SPELL_PYRRHIC_FOCUS))
                                {
                            ...
                            case EVENT_ESSENCE_OF_THE_RED:
                                if (!me->FindNearestCreature(NPC_PULSATING_EGG, 200.0f, true))
                                {
                                    ...
                                    Trinity::UnitListSearcher<Trinity::AnyFriendlyUnitInObjectRangeCheck> searcher(me,
                                            TargetList, checker);
                                ...
                                if (Creature* egg = me->FindNearestCreature(NPC_PULSATING_EGG, 200.0f, true))
                                    if (!egg->HasAura(SPELL_TWI_CARAPACE))
                                    {
                                        if (egg->HealthAbovePct(75) && talkcount == 0)
                                        {
                                            ...
                                            me->GetMotionMaster()->MovePoint(1, MagicBattlePos[1].GetPositionX(),
                                                    MagicBattlePos[1].GetPositionY(), MagicBattlePos[1].GetPositionZ());
                                            talkcount++;
                                        }
                                        if (egg->HealthBelowPct(75) && talkcount == 0)
                                        {
                                            ...
                                            me->GetMotionMaster()->MovePoint(1, MagicBattlePos[0].GetPositionX(),
                                                    MagicBattlePos[0].GetPositionY(), MagicBattlePos[0].GetPositionZ());
                                            talkcount++;
                                        }
                                        if (egg->HealthAbovePct(65) && talkcount == 1)
                                        {
                                            ...
                                            me->GetMotionMaster()->MovePoint(1, MagicBattlePos[2].GetPositionX(),
                                                    MagicBattlePos[2].GetPositionY(), MagicBattlePos[2].GetPositionZ());
                                            talkcount++;
...
{
    public:
        spell_sinestra_barrier() :
                SpellScriptLoader("spell_sinestra_barrier")
        {
        }
        ...
        class spell_sinestra_barrier_AuraScript: public AuraScript
        {
                ...
                {
                    OnEffectPeriodic +=
                            AuraEffectPeriodicFn(spell_sinestra_barrier_AuraScript::HandlePeriodicTick, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
                }
        };
...
{
    public:
        spell_twilight_essence() :
                SpellScriptLoader("spell_twilight_essence")
        {
        }
        ...
        class spell_twilight_essence_SpellScript: public SpellScript
        {
                ...
                {
                    targets.remove_if(
                            ExactDistanceCheck(GetCaster(), 2.5f * GetCaster()->GetFloatValue(OBJECT_FIELD_SCALE_X)));
                }

                void Register()
                {
                    OnObjectAreaTargetSelect +=
                            SpellObjectAreaTargetSelectFn(spell_twilight_essence_SpellScript::ScaleRange, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
                    OnObjectAreaTargetSelect +=
                            SpellObjectAreaTargetSelectFn(spell_twilight_essence_SpellScript::ScaleRange, EFFECT_1, TARGET_UNIT_DEST_AREA_ENTRY);
                }
        };
...
{
    public:
        TwilightSlicerSelector(Unit* caster, Unit* cutterCaster) :
                _caster(caster), _slicerActive(cutterCaster)
        {
        }
...
{
    public:
        spell_twilight_slicer() :
                SpellScriptLoader("spell_twilight_slicer")
        {
        }
        ...
        class spell_twilight_slicer_SpellScript: public SpellScript
        {
                ...
                {
                    OnObjectAreaTargetSelect +=
                            SpellObjectAreaTargetSelectFn(spell_twilight_slicer_SpellScript::RemoveNotBetween, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
                }
        };
...
class spell_pyrrhic_focus: public SpellScriptLoader
{
    public:
        spell_pyrrhic_focus() :
                SpellScriptLoader("spell_pyrrhic_focus")
        {
        }
        ...
        class spell_pyrrhic_focus_AuraScript: public AuraScript
        {
                ...
                {
                    OnEffectPeriodic +=
                            AuraEffectPeriodicFn(spell_pyrrhic_focus_AuraScript::HandlePeriodicTick, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
                }
        };
        ...
        class spell_wrack_AuraScript: public AuraScript
        {
                ...
                void HandleAfterDispel(DispelInfo* dispelInfo)
                {
                    ...
                    if (AuraEffect* wrackDOT = GetEffect(EFFECT_0))
                    {
                        ...
                        Trinity::UnitListSearcher<Trinity::AnyGroupedUnitInObjectRangeCheck> searcher(target, allies,
                                checker);
                        caster->VisitNearbyObject(40.0f, searcher);

                        if (!allies.empty())
                            for (std::list<Unit*>::const_iterator itr = allies.begin();
                                    (itr != allies.end() && count <= 2); ++itr)
                ...
                {
                    AfterEffectApply +=
                            AuraEffectApplyFn(spell_wrack_AuraScript::HandleAfterEffectApply, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
