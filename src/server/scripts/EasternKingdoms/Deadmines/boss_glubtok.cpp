...
enum Events
{
    ...
    EVENT_SAY_FIREWALL,
    EVENT_SUMMON_BLOSSOM, // I figure it is not timing in which.
...
public:
    ...
    struct boss_glubtokAI : public BossAI {
        ...
        {
            _Reset();
            _lastElement = Frost; //Starts always with flame fists
            _phase2 = false;
            _dying = false;
        ...
        {
            if (victim != me) // He kills himself after all, even.
                Talk(SAY_KILL);
        }
        ...
        G3D::Vector3 GenerateTargetForBeamBunny(bool left)
        {
            ...
            for (uint8 i = 0; i < 8; ++i) // Do not try to generate a position that is too close to us it.
            {
                if (left)
        ...
        void MovementInform(uint32 type, uint32 id)
        {
            ...
            if (id == POINT_FALL_GROUND)
            {
                ...
                me->CastWithDelay(2000, me, SPELL_ARCANE_OVERLOAD_SUICIDE, true);
                me->CastWithDelay(1000, me, SPELL_ARCANE_OVERLOAD_BOOM, true);
            }
        }
...
public:
    ...
    struct npc_glubtok_main_platterAI : public Scripted_NoMovementAI
    {
        ...
        void UpdateAI(uint32 const diff)
        {
            if (_turnTimer <= diff)
            {
                me->SetFacingTo(me->GetOrientation() - M_PI/150); //Hacky. Actually there is for splines, which are held with packages in sync.
                _turnTimer = 100;
            }
            else
                _turnTimer -= diff;
...
class spell_glubtok_generic_proc : public SpellScriptLoader
{
    public:
        ...
        class spell_glubtok_generic_proc_AuraScript : public AuraScript
        {
            ...
            {
                if (GetTarget()->HasSpellCooldown(GetSpellInfo()->Id))
                    return false;

                GetTarget()->AddSpellCooldown(GetSpellInfo()->Id, 0, time(NULL) + 4);
                return true;
            }
...
class FirewallCooldownCheck
{
    public:
        ...
        {
            //if (target->FindNearestCreature(_source->GetEntry(), 10.0f) != _source)
            if (target->ToUnit() && target->ToUnit()->HasSpellCooldown(SPELL_FIRE_WALL_TRIGGERED))
                return true;

            if (target->ToUnit())
                target->ToUnit()->AddSpellCooldown(SPELL_FIRE_WALL_TRIGGERED, 0, time(NULL) + 1);
            return false;
        }
};
