...
public:
    ...
    struct boss_vanessa_vancleefAI : public BossAI
    {
        ...
        void EnterCombat(Unit* who)
        {
            ...
            me->MonsterYell(COMBAT_START, 0, 0);
        ...
        void DamageTaken(Unit* done_by, uint32& damage)
        {
            ...
            if (me->GetHealth() <= damage)
            {
                ...
                if (!Killed)
                {
                    ...
                    me->MonsterTextEmote(HUGH_BOMB, PlayerGUID, true);
                    me->RemoveAllAuras();
                    me->AttackStop();
        ...
        void RopeReady()
        {
            me->MonsterWhisper(VANESSA_DETONATE, PlayerGUID, true);

            for (SummonList::const_iterator iter = ropeList.begin(); iter != ropeList.end(); ++iter)
            {
                if (Creature* rope = Unit::GetCreature(*me, *iter))
                {
                    if (rope->isAlive())
                    {
