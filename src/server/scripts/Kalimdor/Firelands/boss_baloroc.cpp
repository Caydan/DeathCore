...
enum Yells
{
    SAY_AGGRO = 0, SAY_DEATH = 1, SAY_KILL = 2,
    SAY_DECIMATION = 3, SAY_INFERNO = 4, SAY_CRYSTAL = 5, SAY_COUNTDOWN = 6,
    SAY_DECI_ANN = 7, SAY_INFE_ANN = 8, SAY_CRY_ANN = 9,
    SAY_BERSERK = 10,
};
    ...
    public:
        ...
        struct boss_balorocAI: public BossAI
        {
                ...
                void Reset()
                {
                    ...
                    if (GameObject* balorocdoor = me->FindNearestGameObject(GOB_DOOR_BALOROC, 150.0f))
                        me->RemoveGameObject(balorocdoor, true);
                }
...
class npc_shard_of_torment: public CreatureScript
{
    public:
        npc_shard_of_torment() :
                CreatureScript("npc_shard_of_torment")
        {
        }
        ...
        {
                npc_shard_of_tormentAI(Creature* creature) :
                        ScriptedAI(creature)
                {
                    ...
                    while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
                            ...
                            case EVENT_CHECK_RANGE:
                                ...
                                events.ScheduleEvent(EVENT_CHECK_RANGE, 500);
                                break;

                            case EVENT_WAVE_OF_TORMENT:
                                DoCast(me,
                                        RAID_MODE(SPELL_WAVE_OF_TORMENT_10N, SPELL_WAVE_OF_TORMENT_25N,
                                                SPELL_WAVE_OF_TORMENT_10H, SPELL_WAVE_OF_TORMENT_25H));
                                break;
                        }
                    }
                }
...
{
    public:
        spell_baloroc_countdown() :
                SpellScriptLoader("spell_baloroc_countdown")
        {
        }
        ...
        class spell_baloroc_countdown_SpellScript: public SpellScript
        {
                ...
                void Register()
                {
                    OnObjectAreaTargetSelect +=
                            SpellObjectAreaTargetSelectFn(spell_baloroc_countdown_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ALLY);
                }
        };
...
{
    public:
        spell_baloroc_countdown_dmg() :
                SpellScriptLoader("spell_baloroc_countdown_dmg")
        {
        }
...
{
    public:
        spell_baloroc_countdown_dmg_target() :
                SpellScriptLoader("spell_baloroc_countdown_dmg_target")
        {
        }
        ...
        class spell_baloroc_countdown_dmg_SpellScript: public SpellScript
        {
                ...
                {
                    OnObjectAreaTargetSelect +=
                            SpellObjectAreaTargetSelectFn(spell_baloroc_countdown_dmg_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ALLY);
                }
        };
