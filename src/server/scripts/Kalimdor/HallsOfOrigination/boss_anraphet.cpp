    ...
    struct npc_whirling_winds_hooAI : public ScriptedAI
    {
        ...
        void SpellHit(Unit * target, const SpellInfo * spell)
        {
            if(target->GetGUID() == currentTarget && (spell->Id == SPELL_WHIRLING_WINDS_EFF || spell->Id == SPELL_WHIRLING_WINDS_EFF_H))
            {
                ...
                if(!targetGUIDs.empty())
                {
                    for(std::list<uint64>::const_iterator itr = targetGUIDs.begin(); itr != targetGUIDs.end(); ++itr)
                    {
                        Unit * target = me->GetUnit(*me, (*itr));
                        if(target && target->isAlive())
                        {
                            currentTarget = (*itr);
                            me->GetMotionMaster()->Clear();
...
public:
    ...
    {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_EVENT, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);

        player->SEND_GOSSIP_MENU(BRANN_TEXTID, creature->GetGUID());
        return true;
    }
